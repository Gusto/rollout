...
RSpec.describe "Rollout" do
  let(:redis) { Redis.new }
  let(:rollout) { Rollout.new(redis, rollout_options) }
  let(:rollout_options) { Hash.new }
  ...
  describe "deactivating a group" do
    ...
    context 'when using sets' do
      let(:rollout_options) { {use_sets: true} }
  ...
  describe "activating a feature for a percentage of users" do
    ...
    context 'when randomizing percentages' do
      let(:rollout_options) { {randomize_percentage: true} }

      it "activates the feature for a random set of users" do
    ...
    it "activates the feature for the same set of users" do
      chat_users = (1..100).select { |id| rollout.active?(:chat, double(id: id)) }
      beta_users = (1..100).select { |id| rollout.active?(:beta, double(id: id)) }
